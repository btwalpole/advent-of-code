...
//convert each string into an array, separated by the spaces]

let newArray = []

for(let i = 0; i<array.length; i++) {
    newArray[i] = array[i].split(' ');
    newArray[i][1] = newArray[i][1].slice(0,1)
    newArray[i][0] = newArray[i][0].split('-')
    console.log(newArray[i])
}

//onsole.log(newArray[0])
...
for(let i = 0; i<newArray.length; i++) {
    let pw = newArray[i][2]
    let validChar = newArray[i][1]
    let lowerBound = newArray[i][0][0]
    let upperBound = newArray[i][0][1]

    console.log('letter: ' + validChar + ', lowerBound: ' + lowerBound + ', upperBound: ' + upperBound)
    console.log('pw :', pw)

    let charCount = 0;
    for(let j=0;j<pw.length;j++) {
        if (pw.charAt(j) === validChar) {
            charCount++;
        }
    }
    console.log('charCount: ', charCount)
    if (charCount<=upperBound && charCount>=lowerBound) {
        validCount++;
    }
...
for(let i = 0; i<newArray.length; i++) {
    let pw = newArray[i][2]
    let validChar = newArray[i][1]
    let validPos1 = newArray[i][0][0]
    let validPos2 = newArray[i][0][1]

    console.log('letter: ' + validChar + ', validPos1: ' + validPos1 + ', validPos2: ' + validPos2)
    console.log('pw :', pw)

    if(pw.charAt(validPos1-1) == validChar && pw.charAt(validPos2-1) != validChar) {
        validCount++
    } else if (pw.charAt(validPos2-1) == validChar && pw.charAt(validPos1-1) != validChar) {
        validCount++
    }
