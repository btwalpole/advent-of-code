...
function doStuff(array) {
    ...
    for(let i = 0; i<array.length; i++) {
       console.log('i: ', i)
        let tryPos = i;
        console.log('trying as minFuelPos: ', tryPos)
        if (!usedPos.includes(tryPos)) {
            usedPos.push(tryPos)
            let sum = 0;
            for(let j= 0; j<array.length; j++) {
                diff = Math.abs(array[j] - tryPos)
                sum = sum + diff
                //console.log('at j: ' + j + ' , diff between ' + tryPos + ' and ' + array[j] + ': ' + diff)
                //console.log('so sum is: ', sum)
            }
            if (i===0) {
                leastFuelPos = tryPos
                leastFuelValue = sum;
                //console.log('first attempt so leastFuelPos: ' + leastFuelPos + ', leastFuelValue: ' + leastFuelValue)
            } else {
                if (sum < leastFuelValue) {
                    //console.log('sum for tryPos ' + tryPos + ': ' + sum + ', < ' + leastFuelValue)
                    leastFuelPos = tryPos
                    leastFuelValue = sum;
                }
                console.log('leastFuelPos: ' + leastFuelPos + ', leastFuelValue: ' + leastFuelValue)
            }
        }
        
    }

    console.log(leastFuelPos)
}
       ...
       //console.log('i: ', i)
        ...
        if (!usedPos.includes(tryPos)) {
            ...
            for(let j= 0; j<array.length; j++) {
                ...
                //console.log('diff = ' + array[j] + ' - ' + tryPos)
                diff = Math.abs(array[j] - tryPos)
                let fuelUsed = diff;
                for(let k = diff-1; k>0;k--) {
                    //console.log('k: ', k)
                    //console.log('fuelUsed = ' + fuelUsed +  ' + ' + k)
                    fuelUsed = fuelUsed + k;
